...
-- 8. Show the institution, the total sample size and the number of computing students for
--    institutions in Manchester for 'Q01'.

SELECT institution, SUM(sample), (SELECT SUM(sample)
                                    FROM nss n1
                                    WHERE question = 'Q01'
                                    AND subject = '(8) Computer Science'
                                    AND n1.institution = n2.institution) AS comp
  FROM nss n2
  WHERE question = 'Q01'
  AND institution LIKE '%Manchester%'
  GROUP BY institution;